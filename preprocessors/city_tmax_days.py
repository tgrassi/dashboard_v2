...
def preprocess():
    ...
    for city in cities:
        ...
        tmax_days = []
        for year in years:
            count_tmax = df[df['date'].dt.year == int(year)]['temperature_2m_max'].gt(tmax_threshold).sum()
            tmax_days.append(int(count_tmax))
        ...
        # compute moving average of tmax_days
        window_size = 10
    ...
    with open("website/data/city_tmax_days.json", "w") as f:
