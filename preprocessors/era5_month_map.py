...
def preprocess():
    ...
    for y in sorted(np.unique(years)):
        if year_avg_min <= y <= year_avg_max:
            temp_mean.append(np.mean(temp[(years == y) & (months == last_month), :, :], axis=0))

    temp_mean = np.mean(temp_mean, axis=0)

    temp_mean_last = np.mean(temp[(years == last_year) & (months == last_month), :, :], axis=0)
    ...
    month_name = MONTHS_NAME[int(last_month) - 1]
    layout = {
        "width": 600,
        "height": 600,
        "title": {"text": f"{month_name} {last_year}: qual è l'anomalia di temperatura ({year_avg_min}-{year_avg_max})?"},
             }

    # first layout so it is easier to debug in the json file
    bundle = {"layout": layout, "data": data}

    with open("website/data/era5_monthly_map.json", "w") as f:
